...
class AttentionScorer:
    ...
    def eval_scores(
        ...
        # Update distracted time based on the head pose
        # if ((head_roll is not None and abs(head_roll) > self.roll_thresh) or (
        #         head_pitch is not None and abs(head_pitch) > self.pitch_thresh) or (
        #         head_yaw is not None and abs(head_yaw) > self.yaw_thresh)):
        #     self.distracted_time = t_now - self.last_time_attended
        ...
        if self.verbose:  # print additional info if verbose is True
            # print(
            #     f"Closure Time: {self.closure_time:.2f}s (Threshold: {self.ear_time_thresh:.2f}s), Asleep: {asleep}")
            # print(
            #     f"Not Look Ahead Time: {self.not_look_ahead_time:.2f}s (Threshold: {self.gaze_time_thresh:.2f}s), Looking Away: {looking_away}")
            # print(
            #     f"Distracted Time: {self.distracted_time:.2f}s (Threshold: {self.pose_time_thresh:.2f}s), Distracted: {distracted}")
            # print(f"Eye closure time: {self.closure_time}")
            pass
